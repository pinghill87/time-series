...
### Final ARMA(2,1)–GARCH(2,1) model
...
### C. ARMA Mean Model Selection (BTC)
```{r}
...
### D. Mean Model Residuals & ARCH Effects (BTC)
```{r}
...
Volatility clustering is clearly present. SPX displays high volatility persistence (GARCH), Bitcoin reacts strongly to shocks with shorter-lived volatility, and VIX shows extreme jumpiness consistent with stress events. GARCH results confirm that returns are hard to predict, but volatility is highly structured and persistent.