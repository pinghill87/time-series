...
## Bitcoin Daily Log Returns with FTX Collapse
...
## Interrupted Time Series Model of Bitcoin Returns
```{r}
#| warning: false
#| message: false
...
The return distributions before and after the FTX collapse are nearly identical. Medians and overall spread remain the same, with extreme outliers present in both periods. Although large negative returns are slightly less frequent after the event, the difference is minor. This aligns with the regression results, showing no meaningful change in average returns or risk.
