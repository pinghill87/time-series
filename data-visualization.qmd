...
if (length(heat_avail) >= 2 && nrow(returns_xts) > 40) {
  ...
  if (length(rows) > 0) {
    ...
    n_cols <- ncol(t(mat))
    label_step <- 6   # show every 6th window
...
required <- c("Bitcoin", "SP500", "USD")

if (all(required %in% colnames(prices_xts))) {


  btc_m <- xts::to.monthly(prices_xts[, "Bitcoin"],
                           indexAt = "lastof", OHLC = FALSE)

  spx_m <- xts::to.monthly(prices_xts[, "SP500"],
                           indexAt = "lastof", OHLC = FALSE)
  ...
  ggplot(df, aes(Date)) +
    ...
    labs(
      title = "Bitcoin & S&P 500 vs USD Dollar Strength",
      subtitle = "Monthly data, standardized (z-scores)",
      x = NULL,
      y = "Z-score",
      color = NULL
    ) +
